...
kept separate from the MCP product app (which lives at `mcp.arclat.com`).
...
| `mcp.arclat.com`   | The MCP product / dashboard  |

All CTAs point at the MCP app via `lib/site.ts`. Override the target with
...
4. (Optional) Set `NEXT_PUBLIC_MCP_URL` if the MCP app isn't at `mcp.arclat.com`.
